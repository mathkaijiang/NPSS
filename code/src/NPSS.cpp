...
void critical_point()
{
    ...
    get_initial_value(v1,4,91);
    get_initial_value(v2,4,89);
    get_initial_value(v,4,90);
    ...
    FuncsLinear2Cplx(rhoReal1, cplxDofs,1.0, rhoReal,0.05, v);//Empirical disturbance
    //FuncsLinear2Cplx(rhoReal1, cplxDofs,1.0, rhoReal,-0.2, option1->V[k0]);//eigenvector direction disturbance    
    //copy_option_V(option1->V[k0],option2,0);
    printf("\n hhh2 \n");
    for(int i=0;i<cplxDofs;i++)
    {
        //option2->V[0][i][0] = option1->V[k0][i][0];
        option2->V[0][i][0] = v[i][0];
        option2->V[0][i][1] = 0.0;
    }
    fftw_complex **V0;
    ...
    for(int i=0;i<k0;i++)
    {
    	V0[i] = (fftw_complex *)malloc(sizeof(fftw_complex) * cplxDofs);
    }    
    ...
    hiosd_lobpcg_sieh_V0(rhoReal1,option2,V0,k0,ep);
...
void hiosd_lobpcg_sieh_V0(fftw_complex *x, Option *option,fftw_complex **V0,int k0, double ep)
{
    ...
    while(iter < maxiter)
    {
        ...
        for(int i=0;i<cplxDofs;i++)
        {
            ...
            //rhoCplx1[i][1]=( rhoCplx1[i][1]+bta*nlnF[i][1] - bta*tmppF[i][1])/(1+bta*epmckt2[i]);
            rhoCplx1[i][1] = 0.0; //Consider central symmetry
        }
        setCplxZero(rhoCplx1[0]);
        ...
        if(iter % 1 == 0)
        {
            printf("\n %d \t bta = %.3e,  res = %.3e,  Rayleigh quotient = %.3e \n",iter, bta, res, alpha[0]);
        }
